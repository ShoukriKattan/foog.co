...
- (void)setTooltipVisible:(BOOL)tooltipVisible animated:(BOOL)animated atTouchPoint:(CGPoint)touchPoint
{
    ...
    dispatch_block_t adjustTooltipPosition = ^{
        ...
        if (self)
        {
            CGFloat minChartX = (ceil(self.tooltipView.frame.size.width * 0.5));
            if (convertedTouchPoint.x < minChartX)
            {
                convertedTouchPoint.x = minChartX;
            }
            CGFloat maxChartX = (self.frame.size.width - ceil(self.tooltipView.frame.size.width * 0.5));
            ...
            CGFloat minTipX = (self.tooltipTipView.frame.size.width);
            if (originalTouchPoint.x < minTipX)
            {
                originalTouchPoint.x = minTipX;
            }
            CGFloat maxTipX = (self.frame.size.width - self.tooltipTipView.frame.size.width);
